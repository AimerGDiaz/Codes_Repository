...
Another great solution is by using awk -F command, whihc allow us to split flieds using regrex expression, in this case :
```{bash,  eval=FALSE}
...
Each time the mother folder is modified, you should run again the commnad, rsync remote-update protocol will  update the file by sending only those different to the already syncronized files. 
